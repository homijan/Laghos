...
void AdvectionBGKMassIntegrator::AssembleElementMatrix
( const FiniteElement &el, ElementTransformation &Trans,
  DenseMatrix &elmat )
{
   AssembleElementMatrix2(el, el, Trans, elmat);
...
void AdvectionBGKMassIntegrator::AssembleElementMatrix2(
   const FiniteElement &trial_fe, const FiniteElement &test_fe,
   ElementTransformation &Trans, DenseMatrix &elmat)
{
   int tr_nd = trial_fe.GetDof();
   ...
   for (int i = 0; i < ir->GetNPoints(); i++)
   {
      ...
      for (int i = 0; i < te_nd; i++)
         for (int j = 0; j < tr_nd; j++)
         {
            elmat(i, j) += w * te_shape(i) * shape(j);
         }
...
void AdvectionBGKNGradIntegrator::AssembleElementMatrix(
...
void AdvectionBGKGammaImIntegrator::AssembleFaceMatrix(
   const FiniteElement &el1, const FiniteElement &el2,
   FaceElementTransformations &Trans, DenseMatrix &elmat)
{
   int dim, ndof1, ndof2, ndof;
   ...
   for (int p = 0; p < ir->GetNPoints(); p++)
   {
      ...
      w = ip.weight * (a+b); // ndotnGamma1 < 0
      if (w != 0.0)
      {
         for (int i = 0; i < ndof1; i++)
		    for (int j = 0; j < ndof1; j++) 
            {
               elmat(i, j) += w * shape1(j) * shape1(i);
            }
      }
      ...
      if (ndof2)
      {
         ...
         w = ip.weight * (b-a); // ndotnGamma2 < 0 (ndotnGamma1 > 0) 
         if (w != 0.0)
         {
            for (int i = 0; i < ndof2; i++)
			   for (int j = 0; j < ndof2; j++) 
               {
                  elmat(ndof1+i, ndof1+j) += w * shape2(j) * shape2(i);
               }
         }
      }
   }
...
void AdvectionBGKGammaExIntegrator::AssembleFaceMatrix(
   const FiniteElement &el1, const FiniteElement &el2,
   FaceElementTransformations &Trans, DenseMatrix &elmat)
{
   int dim, ndof1, ndof2, ndof;
   ...
   for (int p = 0; p < ir->GetNPoints(); p++)
   {
      ...
      w = ip.weight * (a+b); // ndotnGamma2 > 0 (ndotnGamma1 < 0)
      ...
      if (ndof2)
      {
         ...
         if (w != 0.0)
            for (int i = 0; i < ndof1; i++)
			   for (int j = 0; j < ndof2; j++) 
               {
                  elmat(i, ndof1+j) -= w * shape2(j) * shape1(i);
               }

         w = ip.weight * (b-a); // ndotnGamma1 > 0
         if (w != 0.0)
         {
            for (int i = 0; i < ndof2; i++)
			   for (int j = 0; j < ndof1; j++) 
               {
                  elmat(ndof1+i, j) -= w * shape1(j) * shape2(i);
               }
         }
      }
   }
...
void AdvectionBGKDomainLFIntegrator::AssembleRHSElementVect(
